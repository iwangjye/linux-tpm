...
static int tpm2_tis_recv(struct tpm_chip *chip, u8 *buf, size_t count)
{
    ...
    wait_for_tpm_stat_c(chip, TPM_STS_VALID, chip->vendor.timeout_c,
    ...
    if (chip->vendor.irq) {
        ...
        if (wait_for_tpm_stat_c
            (chip, TPM_STS_DATA_AVAIL | TPM_STS_VALID,
             tpm_calc_ordinal_duration_c(chip, ordinal),
...
static const struct file_operations tis_ops = {
    ...
    .read = tpm_read_c,
    .write = tpm_write_c,
    .release = tpm_release_c,
};

static DEVICE_ATTR(pcrs, S_IRUGO, tpm2_show_pcrs_c, NULL);
static DEVICE_ATTR(ownerauth, S_IRUGO, tpm2_show_ownerauth_c, NULL);
static DEVICE_ATTR(endorseauth, S_IRUGO, tpm2_show_endorseauth_c, NULL);
static DEVICE_ATTR(phenable, S_IRUGO, tpm2_show_phenable_c, NULL);
static DEVICE_ATTR(shenable, S_IRUGO, tpm2_show_shenable_c, NULL);
static DEVICE_ATTR(ehenable, S_IRUGO, tpm2_show_ehenable_c, NULL);
            ...
            resource_size_t len, unsigned int irq)
{
    ...
    if (!(chip = tpm_register_hardware_c(dev, &tpm2_tis)))
        return -ENODEV;
...
out_err:
    ...
    tpm_remove_hardware_c(chip->dev);
    return rc;
}
